...
const unsigned
  ...
  int pos = 0; // indices de lectura, escritura
...
void* funcion_productor(void* ){
    ...
    for(unsigned i = 0; i < num_items; i++){
        ...
        buffer[pos] = dato; // Guardamos el dato en la posicion de escritura
...
void* funcion_consumidor(void* ){
    ...
    for(unsigned i = 0; i < num_items; i++){
        ...
        pos--;
        dato = buffer[pos]; // Sacamos el dato del buffer de la posicion de lectura
...
int main(){
    ...
    sem_init( &puede_producir, 0, 20);   // Inicialmente se puede escribir/producir
