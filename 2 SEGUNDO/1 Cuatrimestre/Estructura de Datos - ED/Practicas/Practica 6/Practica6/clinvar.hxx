  ...
  */
  ...
  mutacion ClinVar::load(string nombreDB){
    mutacion mut;
    string cadena;
  ...
  */
  ClinVar::mut_iterator ClinVar::find_mut(IDmut ID) {
    mut_iterator empty;
    empty.setit(mutDB.begin());
    for(empty; empty.getit() != mutDB.end(); empty++){
      if(empty.getID() == ID){
        return empty;
      }
    }
  ...
  */
  set<mutacion, ClinVar::ProbMutaciones> ClinVar::topKMutaciones(int k, string keyword){
    set<mutacion, ClinVar::ProbMutaciones> res;

    list<IDenf> una;
    una = getEnfermedades(keyword);
    int cont = 0;
    while((una.size() > 0) && (cont < k)){
